...
def make_kernel(mean, scale, diag_bijector=None):
    ...
    Args:
        ...
        diag_bijector: Object with inverse method, for diagonal scale matrix.
    ...
    if len(scale.shape) == 4:
        if diag_bijector is None:
            ...
            return torch.cat([mean, diag_bijector.inverse(scale)], dim=-1)
    elif len(scale.shape) == 5:
        scale_tril = FillScaleTriL(diag_bijector=diag_bijector)
        return torch.cat([mean, scale_tril.inverse(scale)], dim=-1)
    else:
        raise ValueError(f"Invalid scale shape: {scale.shape}")

#  deserialize 的作用是根据 TensorFlow 的版本, 对一个对象进行反序列化。
# def deserialize(obj):
